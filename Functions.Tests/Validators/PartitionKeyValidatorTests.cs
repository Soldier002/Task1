...
{
    public class PartitionKeyValidatorTests
    {
        [Test]
        public async Task GivenCorrectlyFormattedKey_WhenValidate_ReturnsSuccess()
        {
            ...
            var partitionKey = originalDateTime.ToString(DateTimeFormats.PartitionKeyDateFormat);
            var validator = new PartitionKeyValidator();

            // act
            var result = validator.Validate(partitionKey, nameof(partitionKey));

            // assert
            Assert.That(result.Success, Is.True);
            Assert.That(partitionKey.CompareTo(result.Value) == 0, Is.True);
        ...
        public async Task GivenIncorrectlyFormattedString_WhenValidate_ReturnsFailure()
        {
            // arrange
            var originalDateTime = new DateTime(2020, 9, 8, 10, 11, 12);
            var partitionKey = originalDateTime.ToString("yyyy/MM/dd");
            var validator = new PartitionKeyValidator();
